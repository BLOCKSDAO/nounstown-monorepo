...
import classes from './TrackerTester.module.css';
import { Row, Col } from 'react-bootstrap';
import { getNounSVGBuffer, getRecenttAuctionBids, getAuction, getBlockNumber, getAuctionMinBidIncPercentage, getNextNounSVGBuffer } from '../../utils/trackerUtils';
...
const TrackerTester: React.FC<{ 
  ...
  const { 
  	name, uri, tokenAddress, auctionHouseProxyAddress, 
  	descriptorAddress, seederAddress,
  	subgraphApiUri, 
  ...
  const [nextAuctionSVG, setNextAuctionSVG] = useState<Buffer | null>();
  ...
  */
  ...
  useEffect(() => {
    ...
    console.log('timer');
      ...
      if (descriptorAddress !== undefined && seederAddress !== undefined) {
      	setNextAuctionSVG(await getNextNounSVGBuffer(descriptorAddress, seederAddress, (Number(auctionId) + 1).toString()));
      }
	...
	if (timerType === 'blocks') {
	    ...
	    if (true) { //always run
	      const timer = setTimeout(
	        ...
	        (timeLeft > 300 || timeLeft <= 0) ? 1000 : 1000,
	      );
	      ...
	      <Row className={classes.activityRow}>
	        <Col lg={2} className={classes.currentBidCol}>
				...
				<a href={uri} target="_blank" rel="noreferrer">
		        	...
		        	<Noun 
		        		imgPath={(nextAuctionSVG) ? `data:image/svg+xml;base64,${btoa(nextAuctionSVG.toString())}` : ''} 
		        		alt={name} 
		        		className={classes.nounImg}
		        		wrapperClassName={classes.nounWrapper}
		        	/>
