...
import classes from './TrackerSettleManuallyBtn.module.css';
...
const TrackerSettleManuallyBtn: React.FC<{
  settleAuctionHandler: () => void;
  auctionContract: ContractAuction;
}> = props => {
  const { settleAuctionHandler, auctionContract } = props;
  ...
  useEffect(() => {
    ...
    // prettier-ignore
    const timeLeft = MINS_TO_ENABLE_MANUAL_SETTLEMENT * 60 - (dayjs().unix() - (auctionContract && Number(auctionContract.endTime)));
...
export default TrackerSettleManuallyBtn;
