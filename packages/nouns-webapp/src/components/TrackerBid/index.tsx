...
import classes from './TrackerBid.module.css';
...
const TrackerBid: React.FC<{
  auctionContract: ContractAuction;
  auctionHouseProxyAddress: string;
  auctionEnded: boolean;
  auctioMinBidIncPercentage: BigNumber;
}> = props => {
  const activeAccount = useAppSelector(state => state.account.activeAccount);
  const { library } = useEthers();
  let { auctionContract, auctionHouseProxyAddress, auctionEnded, auctioMinBidIncPercentage } = props;
  ...
  //move this to the at the tracker component level so we have less frequent calls...
  //const minBidIncPercentage = useAuctionMinBidIncPercentage(auctionHouseProxyAddress);
  const minBidIncPercentage = auctioMinBidIncPercentage;
  
  const minBid = computeMinimumNextBid(
    auctionContract && new BigNumber(auctionContract.amount.toString()),
    minBidIncPercentage,
  );
  ...
  const placeBidHandler = async () => {
    if (!auctionContract || !bidInputRef.current || !bidInputRef.current.value) {
      return;
    }
    ...
    const gasLimit = await contract.estimateGas.createBid(auctionContract.nounId, {
      value,
    });
    placeBid(auctionContract.nounId, {
      value,
  ...
  }, [auctionContract, placeBidState, account, setModal]);
